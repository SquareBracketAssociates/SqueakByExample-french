...
L'objectif de ce chapitre est de vous montrer comment d\'evelopper des programmes dans l'environnement de programmation de \Squeak.
...
Squeak a un excellent d\'ebogueur, mais comme la plupart des outils puissants, il peut s'av\'erer d\'eroutant au d\'ebut.
Nous vous en parlerons au travers de session de d\'ebogage et vous
...
La technique plus r\'ecente en \Squeak est le chargement de code dans un d\'ep\^ot de versions sur un serveur.
...
avait un environnement de d\'eveloppement graphique auto-h\'eberg\'e et
tout le d\'eveloppement en Smalltalk se d\'eroulait dessus.
...
Vous l'utiliserez pour cr\'eer, d\'efinir et organiser vos classes et vos m\'ethodes. Avec lui, vous pourrez aussi naviguer dans toutes les classes de la biblioth\`eque de classes: contrairement aux autres environnements o\`u le code source est r\'eparti dans des fichiers s\'epar\'es, en \st toutes les classes et m\'ethodes sont contenues dans l'image.
	...
	\item Le {\menu{Method Finder}} vous permet aussi de trouver des m\'ethodes mais, soit selon leur \emph{action}, soit en fonction de leur nom.
	\index{method finder}
	
	\item Le {\menu{Monticello Browser}} est le point de d\'epart pour le chargement ou la sauvegarde de code via des paquetages \ind{Monticello} dit aussi packages.
...
\menu{do it}~\footnote{En anglais, \emph{do it} correspond \`a l'exclamation ``fais-le!''.}. L'utilisation de \ind{Workspace} a aussi fait l'objet d'une \'etude dans \secref{transcript}.
\end{itemize}

L'outil \menu{Debugger} a un r\^ole \'evident, 
mais vous d\'ecouvrirez qu'il a une place plus centrale en comparaison
des d\'ebogueurs dans d'autres langages de programmation car en \st
vous pouvez \emph{programmer} dans \ind{Debugger}.  Il n'est pas
...
Le premier panneau de gauche liste les \emph{cat\'egories} des classes;
...
Le bouton \button{hierarchy} ouvre un navigateur hi\'erarchique ou \ind{Hierarchy Browser} sur la classe actuelle; ce navigateur
peut aussi \^etre ouvert en utilisant \menu{browse hierarchy} dans le menu du
panneau de classe.
...
Les boutons \button{inst vars} et \button{class vars} vous aide \`a savoir
...
ont tous \menu{file out} dans leur menu respectif. Cependant, chaque 
...
une m\'ethode'') et
\menu{find method wildcard\ldots} qui s'av\`erent utiles si souhaitez
...
dans les paquetages tels qu'arrang\'es dans \ind{Monticello}, cependant
...
Il est influenc\'e par la convention de nommage employ\'ee depuis longtemps et dont nous avons parl\'e plus haut (dans \secref{package-names}); ce syst\`eme enrichit cette convention de mani\`ere cons\'equente.
...
\ct{SqueakLink} et avez cr\'e\'e une s\'erie de cat\'egories-syst\`eme
...
La classe \ct{PackageInfo} impl\'emente ces r\`egles et vous pouvez 
...
\ct{self} n'est pas li\'e \`a \`a l'objet racine root mais plut\^ot
...
suffix
	"disons que je suis un nom de fichier et que je fournis mon suffixe, parie suivant le dernier point"
	| dot dotPosition |
...
S'en suit l'ex\'ecution de la m\'ethode \ct{suffix} h\'erit\'ee dans la
classe \ct{String};
...
\ct{'readme.txt'} \`a la p\'enulti\`eme (soit l'avant-avant-derni\`ere) 
...
Le commentaire dans la m\'ethode \ct{suffix} n'est pas tr\`es pr\'ecise.
% note de martial: commentaire traduit car en reference
La fa\c{c}on d'\'eviter cette sorte de probl\`eme est d'\'ecrire
un test \ind{SUnit} pour d\'efinir en fois pour toute la r\'eponse.
...
C'est tout pour le d\'ebogueur mais nous n'en avons pas fini de la m\'ethode \ct{suffix}.
...
\index{navigateur de noms de message}
...
\index{navigateur de change set}
...
\ct{ChangeList browseRecent: 2000} en tantonnant sur le chiffre empirique
\ct{2000}).
