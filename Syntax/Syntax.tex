...
\noindent
La syntaxe de \sq est minimaliste; pour l'essentiel, con\c{c}ue uniquement pour \emph{envoyer des messages} (\ie des expressions).
...
Par exemple, \`{a} la place de la structure de contr\^{o}le \emph{if-then-else}, \st envoie des messages comme \ct{ifTrue:} \`{a} des objets booléens de classe
\clsind{Boolean}.
...
Par convention, les identifiants sont compos\'{e}s de mots au format d'écriture \emph{casse de chameau} de la forme ``\ind{camelCase}''(\ie chaque mot except\'{e} le premier d\'{e}bute par une lettre majuscule).
...
\item[Les tableaux \`{a} la compilation.] Ils sont d\'{e}finis par \ct{#( )}, les objets litt\'{e}raux sont s\'{e}par\'{e}s par des espaces.
...
\item[Les tableaux \`{a} l'ex\'{e}cution.] Les accolades \ct|{ }|
...
(la notation entre accolades est particuli\`{e}re \`{a} \sq, dialecte de \st!
...
sp\'{e}ciale.
Ainsi, \ct{x := 1} est identique \`{a} \ct{x _ 1} ou \ct{x UNDERSCORE 1}. Vous devriez utiliser  \ct{:=} puisque les autres repr\'{e}sentations ont \'{e}t\'{e} supprim\'{e}es depuis la version 3.9 de Squeak.
\index{affectation}
...
\ind{retour}nera de fa\c{c}on implicite \pvind{self}.
...
Dans certaines situations, ce probl\`{e}me peut entraîner des conflits de nommage;
Cela se produit car \sq est construit sur une des premi\`{e}res impl\'{e}mentations de \st.
...
Les clauses conditionnelles sont obtenus par l'envoi des messages
...
Ne perdez pas de vue que les chaînes sont aussi des conditions, ainsi
...
voyelle~\footnote{Note du traducteur: les voyelles accentu\'es ne sont
...
\item	Il y a cinq types d'objets litt\'{e}raux: les nombres (\ct{5},
  \ct{2.5}, \mbox{\ct{1.9e15},} \ct{2r111}), les caract\`{e}res (\ct{$a}),%$
