...
\item La \emphind{machine virtuelle} (agr\'eg\'e en VM pour
  ...
  systèmes (Linux, Mac OS X, Win32). Dans \figref{download}, vous
  avez par exemple l'icone de la machine virtuelle pour le syst\`eme Mac OS X: \textit{\sq 3.8.15beta1U.app}.
...
Les images sont très utiles mais on considère comme une pratique un peu dépassée et fragile pour diffuser et partager vos projets alors qu'il existe des outils tels que Monticello qui proposent de biens meilleurs moyens de suivre les évolutions du code et de le partager entre plusieurs développeurs.
...
L'onglet s'élargira et présentera les icônes de certains outils importants de \sq (\figref{tools}). Faire un glisser-déposer de l'icône \ind{Transcript} puis l'icône \ind{Workspace}.
...
n'importe quel zone de texte) et tapez \short{b}.
...
\caption{Recherche d'une classe d'apr\`es son nom.\label{fig:findit}}
\end{figure}
