...
mod\`ele centr\'e sur le comportement~\footnote{En anglais,
  \emph{Behaviour Driven Development}.} se r\'esume par le fait
d'\'ecrire un test qui d\'efinit le comportement d\'esir\'e de notre
code \emph{avant} d'\'ecrire le code proprement dit.
...
\begin{quote}
When we send the message \ct{shout} to the string ``Pas de panique''
the result should be ``PAS DE PANIQUE!''.
\end{quote}
...
\begin{itemize}
  ...
  \item Le \emph{Test Runner} permet d'effectuer des tests
    ...
    programmation orient\'ee tests connus sous le nom de \emph{Test
      Driven Development}.
\end{itemize}
