...
%% Version: 13642
%% Last-Updated: Mon Jan 18 17:27:49 2010 (+0100)
%%           By: Martial Boniou
%%     Update #: 771
%% URL:
...
différents systèmes d'exploitation; en conséquence de quoi, \sq
...
des autres boîtes \`a outils graphiques: tous ses éléments
graphiques peuvent être assemblés et désassemblés \`a tout
...
souris ou en programmation), \`a leur ajouter des fonctionnalités (pour
...
\dothis{Afficher le menu World. Cliquez une première fois avec le
  \ind{bouton bleu} de la souris de manière \`a afficher le
  ...
  de menu que vous voulez détacher, disons \menu{new
    morph\ldots}. Le halo est donc affiché pour cet élément du
...
\ct{asMorph} dans \ct{Object} fait appel \`a sa méthode dérivée
...
remplaçant la programmation par une simple manipulation \`a la souris.
...
morph \`a l'écran. Le paramètre passé \`a \ct{drawOn:} est un
...
Le résultat devrait être semblable \`a celui de \figref{cross}.
...
\figref{bug}.
Ceci est du \`a un problème de troncature: %temporaire  
...
\mthind{Morph}{mouseUp:}  \`a ces même morphs qui avaient répondus
...
\dothis{Ajoutez la méthode suivante \`a la classe \ct{CrossMorph}:}
...
Occupons-nous de \ct{CrossMorph} pour que nos croix réagissent \`a
...
\noindent
Ce message est équivalent \`a \mthind{Morph}{handlesMouseDown:}
...
simples animations grâce \`a quatre méthodes:
\begin{itemize}
\item \mthind{Morphic}{step} qui est envoyé au morph \`a un
...
\item \mthind{Morphic}{stepTime} qui définit l'intervalle de temps en
  millisecondes entre chaque envoi du message \ct{step}~\footnote{\ct{stepTime} est
    ...
    1\,ms, ne soyez pas étonné si \sq est trop occupé pour que
...
\`A ces méthodes s'ajoutent une méthode de test
...
un \menu{do it}.
%ajout
Pour arrêter l'animation, vous n'avez qu'\`a évaluer \ct{self stopStepping} dans l'inspecteur. 
...
\begin{minipage}{0.38\textwidth}
	...
	\vfill
	\vspace{4ex}
	\caption{Un menu flottant affiché grâce \`a \ct{PopUpMenu>>>startUpWithCaption:}.}
\end{minipage}
\end{figure}
...
\dothis{Changez la couleur de l'ellipse pour du bleu via l'inspecteur
  ...
  le \ct{ReceiverMorph}: essayez \`a nouveau le glisser-déposer.}
...
L'envoi du message \mthind{MorphicEvent}{hand} \`a un événement
répond la ``main'' (en anglais, \emph{hand}), instance de
\ct{HandMorph} qui représente le pointeur de la souris et tout ce
qu'il tient.
Dans notre méthode, nous disons \`a l'écran \sq, \ct{World}, que
la main 
%ajout
...
dé dans une boucle rapide suite \`a un premier clic de
...
\begin{enumerate}
	...
	\item \ct{DieMorph class>>>new} retourne \`a la méthode de
...
%Avant de passer \`a la définition de la méthode \ct{drawOn:}
