...
\sunit a été développé par Ken Beck et ensuite grandement étendu par d'autres développeurs, dont notamment Joseph Pelrine, avec la prise en compte de la notion de ressource décrite dans la \secref{resource}. 
...
Dans ce chapitre, nous commençons par discuter de pourquoi nous testons et de quoi est fait un bon test. Nous présentons alors une séries de petits exemples montrant comment utiliser \sunit. Finalement, nous étudions l'implémentation de \sunit, de façon à ce que vous compreniez comme \st utilise la puissance de la \ind{réflexivité} pour la mise en oeuvre de ses outils.
...
Ensuite, \sunit dispose de deux méthodes supplémentaires, \mthind{TestCase}{should:raise:} et \mthind{TestCase}{should:raise:}  pour la propagation des exceptions de test. Par exemple, vous utiliseriez \ct{(self should: aBlock raise: anException)} pour tester si une exception particulière est levée pendant l'exécution de \ct{aBlock}. \Mthref{ESTtestIllegal} illustre l'utilisation de \mbox{\ct{should:raise:}}.
...
\item[Tests unitaire \textit{vs.}\ \aconfirmer{tests de qualification}.] 
Des tests unitaires capturent une partie de fonctionnalité et, comme tels, permettent de faciliter l'identification des bugs de cette fonctionnalité. Autant que possible, essayez d'avoir des tests unitaires pour chaque méthode pouvant potentiellement poser problème, et regroupez les par classe. Cependant, pour des situations profondément récursives ou complexes à installer, il est plus facile d'écrire des tests qui représentent un scénario cohérent pour l'application visée; ce sont des tests de \aconfirmer{qualification} ou tests fonctionnels.
Des tests qui violent les principes de  Feathers peuvent faire de bon tests de \aconfirmer{qualification}.
Groupez les tests de \aconfirmer{qualification} en cohérence avec la fonctionnalité qu'ils testent.
Par exemple, si vous écrivez un compilateur, vous pourriez écrire des tests de \aconfirmer{qualification} avec des assertions qui concernent le code généré pour chaque instruction utilisable du langage source.
