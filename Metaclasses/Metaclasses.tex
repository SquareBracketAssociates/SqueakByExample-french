...
Cependant, chaque fois que vous utilisez le System Browser pour naviguer du  «~\subind{System Browser}{côté classe}~» d'une classe, il est utile de se rappeller que vous êtes en train de naviguer dans une classe différente.
...
% \noindent
Ensembles, ces 10 règles complètent le modèle objet de \st.
...
\noindent
Nous pouvons créer une morph et fixer sa couleur à cette couleur translucide:
\begin{code}{}
...
Par la \ref{rule:inheritance}, tout classe possède une super-classe.
...
Comme tout se produit par \ind{envoi de message}{}s (\ref{rule:message}), on peut déduire que \mthind{Color class}{blue} est un message à destination de \ct{Color}, \mthind{Object}{class} et \mthind{Color}{alpha:} sont des messages à destination de la couleur bleue, \mthind{Morph}{openInWorld} est un message à destination d'une ellipse morph, et \mthind{Behavior}{superclass} est un message à destination de \ct{TranslucentColor} et \ct{Color}.
...
La recherche de méthodes suit la chaîne d'héritage (\ref{rule:lookup}), donc quand on envoi le message \ct{class} au résultat de 
...
Notre objet bleu translucide \emphind{est un}{}e instance de \ct{TranslucentColor}, mais on peut aussi dire qu'il
...
La recherche commence dans \ct{TranslucentColor class}, et continue dans la hiérarchie des méta-classes jusqu'à trouver dans  \ct{Color class} (see \figref{metaclasslookup}).
...
Normalement, les objets qui ne sont pas des classes ne comprennent ce message, mais si vous avez une bonne raison pour faire cela, il n'y a rien qui vous empêche d'ajouter une méthode \ct{new} à une classe qui n'est pas une méta-classe.
...
\noindent
Notons que dans un cas, vous inspectez une instance de \ct{Color} et que dans l'autre cas la classe \ct{Color} elle-même.
...
L'inspecteur sur \ct{Color} vous permet de voir la super-classe, les variables d'instances, le \subind{méthode}{dictionnaire} des méthodes, etc., de  la classe \ct{Color}, comme montré dans \figref{inspectingColor}.
...
%=================================================================
\section{Toute m{\'e}ta-classe h{\'e}rite de \lct{Class} et de \lct{Behavior}}
% \ruleref{behavior}
...
\caption{Les méta-classes héritent de \lct{Class} et de \lct{Behavior}\label{fig:inheritbehavior}}
\end{figure}
\end{center}


\paragraph{Où est défini \lct{new}?}
Pour comprendre l'importance du fait que les méta-classes héritent de \ct{Class} et de \ct{Behavior}, cela aide de demander où est défini \ct{new} et comment cette définition est trouvée.
...
\mthind{Behavior}{new} est défini en premier dans la classe \ct{Behavior}, et peut être redéfini dans ses sous-classes, ce qui inclu toutes les méta-classes des classes que nous avons définies, si cela est nécessaire.
...
\caption{\lct{new} est un message ordinaire recherché dans la chaîne des méta-classes.\label{fig:sendingnew}}
\end{figure}
\end{center}
...
Par exemple, \lct{Array new: 4} créer un tableau de 4 éléments.
Vous ne trouverez pas la définition de cette méthode dans \ct{Array} ni aucune de ses super-classes.
...
\paragraph{Les responsabilités de \lct{Behavior}{}, \lct{ClassDescription} et \lct{Class}{}.}
...
Elle fournie des méthodes pour créer un dictionnaire de méthodes, compiler des méthodes, créer des instances (\ie \mthind{Behavior}{new}, \mthind{Behavior}{basicNew}, \mthind{Behavior}{new:}, et \mthind{Behavior}{basicNew:}),
...
\clsind{ClassDescription} ajoute des facilités fournie à la base par \ct{Behavior}:
des variables d'instances nommées,
la catégorisation des méthodes dans des protocoles,
la notion de nom (abstrait),
la maintenance de change sets et la journalisation des changements,
et la plupart des mécanismes requis pour l'exportation de change sets.
...
%=================================================================
\section{Toute méta-classe est une instance de \lct{Metaclass}}
...
\caption{Toute méta-classe est une \lct{Metaclass}.\label{fig:metaclassclass}}
\end{figure}
\end{center}


\ct{Metaclass} représente le comportement commun des méta-classes.
Elle fournit des méthodes pour la création d'instance (\lct{sub\-class\-Of:}) permettant de créer des instances initialisées de l'unique instance \ct{Metaclass}, pour l'initialisation des variables de classe, la compilation de méthodes et l'obtiention d'informations à propos des classes (liens d'héritage, variables d'instance, \etc).
...
\caption{Toutes les méta-classes sont des instances de la classe \lct{Metaclass},  même la méta-classe de \lct{Metaclass}. \label{fig:metaclassclassclass}}
\end{figure}
\end{center}
...
Si vous vous perdez ou vous embrouillez, vous devez toujours vous rappeler que l'envoi de messages est la clée: cherchez alors la méthode dans la classe du receveur.
...
\item Toute méta-classe est une instance de \clsind{Metaclass}.
	\ct{ClassDescription} fournit tout ce qui est commun à \ct{Class} et \ct{Metaclass}.
