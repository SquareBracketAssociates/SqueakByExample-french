...
La recherche de méthodes suit la chaîne d'héritage (\ref{rule:lookup}), donc quand on envoie le message \ct{class} au résultat de 
...
%% etc.
de  la classe \ct{Color}, comme montré dans \figref{inspectingColor}.
...
\mthind{Behavior}{new} est défini en premier dans la classe \ct{Behavior} et peut être redéfini dans ses sous-classes, ce qui inclut toutes les méta-classes des classes que nous avons définies, si cela est nécessaire.
